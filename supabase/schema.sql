-- MaterialMind V3
-- Fresh-install schema. For an existing V2 database, run supabase/migration_v3.sql instead.
...
create unique index if not exists inventory_snapshots_hash_uq on public.inventory_snapshots(source_file_hash);
...
create table if not exists public.inventory_changes (
  ...
  safety_stock numeric not null default 0,
  created_at timestamptz not null default now(),
  unique(to_snapshot_id, part_no, rating, warehouse, storage_bin)
);
create index if not exists inventory_changes_to_idx on public.inventory_changes(to_snapshot_id, change_type);
create index if not exists inventory_changes_bin_idx on public.inventory_changes(to_snapshot_id, warehouse, storage_bin, part_no);
...
create policy audit_admin_read on public.import_audit for select to authenticated
...
begin
  ...
  select count(*) into v_row_count from public.inventory_snapshot_rows
  ...
  if v_previous_id is not null then
    ...
    select count(*) into v_changes from public.inventory_changes
    where to_snapshot_id=v_snapshot_id;
  end if;
...
create or replace function public.agent_heartbeat(
  p_machine_name text,p_status text default 'WAITING',p_last_file text default null,
  p_last_imported_date date default null,p_last_imported_hash text default null,
  p_last_scan timestamptz default now(),p_last_error text default null
)
returns jsonb language plpgsql security definer set search_path=''
as $$
...
begin
  if v_user is null then raise exception 'AUTH_REQUIRED'; end if;
  if not (select public.is_operator_or_admin()) then raise exception 'ROLE_NOT_ALLOWED'; end if;
  insert into public.agent_status(machine_name,status,last_file,last_imported_date,last_imported_hash,
                                   last_scan_at,last_error,updated_by)
  values(p_machine_name,coalesce(p_status,'WAITING'),p_last_file,p_last_imported_date,
         p_last_imported_hash,coalesce(p_last_scan,now()),p_last_error,v_user)
  on conflict(machine_name) do update set
    status=excluded.status,last_file=excluded.last_file,
...
as $$
  ...
  from public.agent_status a where a.machine_name=p_machine_name;
$$;
